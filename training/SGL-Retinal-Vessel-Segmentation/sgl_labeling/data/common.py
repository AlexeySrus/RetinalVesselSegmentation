...
def get_patch(hr, ve_r, ma, patch_size=256, deform = True, train=True, random_toggle=False):
    #hr = process(hr, 1)
    ...
    if train:
        ...
        ret = [hr_t, ve_t, ma, dp, ve_thin, ve_thick]
    else:
    #if True:
        crop_mask = (ve > 0.5).astype(np.uint8) * 255
        x, y, w, h = cv2.boundingRect(crop_mask)

        x = min(x, hr.shape[1] - patch_size - 2)
        y = min(y, hr.shape[0] - patch_size - 2)
        w = max(w, patch_size) + 1
        h = max(h, patch_size) + 1
        ...
        iy = random.randrange(0, ih - ip + 1)
        # crop patch
        hr_img = hr[iy:iy + ip, ix:ix + ip, :]
        ve_img = ve[iy:iy + ip, ix:ix + ip]
        ma_img = ma[iy:iy + ip, ix:ix + ip]

        # ve_img[0, 0] = 1
        # ve_img[0, 1] = 0

        # hr_img = np.zeros((2000, 2000, 3))
        # hr_img[0:ih, 0:iw, :] = hr#process(hr)
        # ve_img = np.zeros((2000, 2000))
        # ve_img[0:ih, 0:iw] = ve
        # ma_img = np.zeros((2000, 2000))
        # ma_img[0:ih, 0:iw] = ma
        ...
        ret = [hr_img, ve_img, ma_img, dp_img, ve_thin, ve_thick]
