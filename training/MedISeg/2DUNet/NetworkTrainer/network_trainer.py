...
from NetworkTrainer.dataloaders.dataload import DataFolder, EyeMaskDataset
...
class NetworkTrainer:
    ...
    def set_network(self):
        ...
        if len(self.opt.train['checkpoint']) > 0:
            self.net.load_state_dict(torch.load(self.opt.train['checkpoint'], map_location='cpu')['state_dict'])
            print('Load from: {}'.format(self.opt.train['checkpoint']))
    ...
    def set_dataloader(self):
        self.train_set = EyeMaskDataset(root_dir=self.opt.root_dir, phase='train', fold=self.opt.fold, gan_aug=self.opt.train['gan_aug'], data_transform=A.Compose(self.opt.transform['train']))
        self.val_set = EyeMaskDataset(root_dir=self.opt.root_dir, phase='val', data_transform=A.Compose(self.opt.transform['val']), fold=self.opt.fold)
