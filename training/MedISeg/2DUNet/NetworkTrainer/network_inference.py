...
class NetworkInference:
    ...
    def set_dataloader(self):
        test_set = EyeMaskDataset(root_dir=self.opt.root_dir, phase='val', data_transform=A.Compose(self.opt.transform['test']), fold=self.opt.fold)
    ...
    def run(self):
        ...
        for i, data in enumerate(tqdm(self.test_loader)):
            ...
            for j in range(pred.shape[0]):
                ...
                if self.opt.test['save_flag']:
                    imageio.imwrite(os.path.join(self.opt.test['save_dir'], 'img', f'{name[j]}.png'), (pred[j] * 255).astype(np.uint8))
                    imageio.imwrite(os.path.join(self.opt.test['save_dir'], 'img', f'{name[j]}_gt.jpg'), (gt[j].numpy() * 255).astype(np.uint8))
