...
def train(dataloader, epoch) -> None:
    ...
    for index, (real, _) in enumerate(dataloader):
        ...
        real = real.to(device).mean(axis=1).unsqueeze(1)
...
def main() -> None:
    ...
    if resume:
        ...
        if resume_d_weight != "" and resume_g_weight != "":
            ...
            tmp_input = torch.randn(1, 100, 1, 1, device=device, requires_grad=True)
            generator.eval()
            traced_generator = torch.jit.trace(generator, tmp_input)
            traced_generator.save('mask_generator.pt')
